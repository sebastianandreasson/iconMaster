...
iconCreator.prototype.download = (url) => {
...
iconCreator.prototype.contrast = amount => {
...
iconCreator.prototype.monochrome = () => {
  const self = this
  self.operationQueue.push(queueCallback => {
...
iconCreator.prototype.checkColor = (x, y) => {
  const self = this
  self.operationQueue.push(queueCallback => {
    x = x ? x : 1
    y = y ? y : 1
...
iconCreator.prototype.resizeAndCenter = size => {
  const self = this
  self.operationQueue.push(queueCallback => {
...
iconCreator.prototype.pixelate = (filter, scale, threshold) => {
  const self = this
  self.operationQueue.push(queueCallback => {
